...
- Plain single-line strings, single- and double-quoted strings, base-10
  integers matching `-?[0-9]+`, `true`, `false`, and YAML null spellings.
...
a reserved YAML indicator, contains `: `, or contains ` &`, ` *`, or ` !` at a
token boundary. Escape a single quote inside a single-quoted string by doubling
it.
